---
name: tanstack-markdown-docs-features
...
This skill builds on [render-markdown](../tanstack-markdown-render-markdown/SKILL.md). Read it first for parser options, AST reuse, rendering, and core trust boundaries.
...
- Highlighter output is trusted HTML. Keep highlighting at build time or on the server and apply the checks in [production-pipelines](../tanstack-markdown-production-pipelines/SKILL.md).
- Import individual extension entry points when the complete preset adds unused behavior or bundle cost.
- Use [custom-extensions](../tanstack-markdown-custom-extensions/SKILL.md) when built-in comment components or transforms do not express the required deterministic syntax.
