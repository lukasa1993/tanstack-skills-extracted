...
Release-matched documentation · `@tanstack/lit-form@1.25.5`.
...
When you split each step into its own custom element, pass the `TanStackFormController` as a property and type it with [`getFormType`](./form-docs-framework-lit-guides-form-composition-md-084681ff.md#source-form-docs-framework-lit-guides-form-composition-md). Because Lit child elements that receive a controller by property do not automatically re-render when the controller state changes, subscribe to `form.api.store` with `TanStackStoreSelector` in the child element.
...
If you want to use [dynamic validation (`onDynamic`)](./form-docs-framework-lit-guides-dynamic-validation-md-65a7cdfb.md#source-form-docs-framework-lit-guides-dynamic-validation-md) with a form group, do not rely on the `onDynamic` validator passed to `TanStackFormController`:
