...
metadata:
  ...
  tanstack-package-version: "0.3.7"
...
The query builder looks like SQL but has constraints that SQL doesn't — equality joins only, orderBy required for limit/offset, no distinct without select. Agents write SQL-style queries that violate these constraints. See ../tanstack-db-core-live-queries/SKILL.md § Common Mistakes for all constraints.

See also: ../tanstack-db-core-live-queries/SKILL.md — for query builder API and all operators.

See also: ../tanstack-db-core-mutations-optimistic/SKILL.md — for mutation patterns.

See also: ../tanstack-db-meta-framework/SKILL.md — for preloading in route loaders.
