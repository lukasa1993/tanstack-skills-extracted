# Custom Marks And Renderers

<a id="source-charts-docs-guides-custom-marks-and-renderers-md"></a>

Release-matched documentation · `@tanstack/charts@0.16.0`.

[Topic index](../composition.md) · [Source provenance](../SOURCES.md)

## Choose a section

Read the overview or setup when it is a prerequisite, then the section needed for the task.

- [Overview](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--overview.md) — 1 KiB
- [Start with composition](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--start-with-composition.md) — 2 KiB
- [Group reusable child marks](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--group-reusable-child-marks.md) — 2 KiB
- [Create a mark](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--create-a-mark.md) — 4 KiB
- [Interaction points](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--interaction-points.md) — 2 KiB
- [Focus-only anchors](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--focus-only-anchors.md) — 1 KiB
- [Focus-guide marks](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--focus-guide-marks.md) — 3 KiB
- [Separate point and scale values](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--separate-point-and-scale-values.md) — 1 KiB
- [Custom scales and legends](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--custom-scales-and-legends.md) — 1 KiB
- [Custom renderer](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--custom-renderer.md) — 3 KiB
- [Custom SVG serializer](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--custom-svg-serializer.md) — 2 KiB
- [Custom focus and spatial indexes](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--custom-focus-and-spatial-indexes.md) — 1 KiB
- [Extension checklist](./charts-docs-guides-custom-marks-and-renderers-md-88f45847--extension-checklist.md) — 1 KiB

<!-- Original source anchors retained for inbound links. -->
<a id="start-with-composition"></a>
<a id="group-reusable-child-marks"></a>
<a id="create-a-mark"></a>
<a id="interaction-points"></a>
<a id="focus-only-anchors"></a>
<a id="focus-guide-marks"></a>
<a id="separate-point-and-scale-values"></a>
<a id="custom-scales-and-legends"></a>
<a id="custom-renderer"></a>
<a id="custom-svg-serializer"></a>
<a id="custom-focus-and-spatial-indexes"></a>
<a id="extension-checklist"></a>
